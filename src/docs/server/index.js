...
const contactData = [
  {
    ...
    company: 'JDLT',
    phone: '123',
    dob: new Date(2019, 1, 1, 10, 33, 30, 0),
    active: false
  },
  {
    firstName: 'Fabian',
  ...
  },
  {
    ...
    company: 'JDLT',
    phone: '321',
    dob: new Date(2019, 2, 6, 10, 33, 30, 0),
    active: true
  },
  {
    firstName: 'Rich',
...
const schema = gql`
  type Query {
    list(offset: Int, limit: Int, filters: FilterInput, sort: SortInput): Data
    getDataCount(offset: Int, limit: Int, filters: FilterInput, sort: SortInput): Int
  }
  ...
  Query: {
    list: (parent, { offset, limit, filters, sort }) => {
      ...
      return filteredData.length > offset
        ? {
            data: sortData(filteredData, sortBy, direction).slice(offset, limit + offset),
